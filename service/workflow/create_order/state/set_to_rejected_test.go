...
func TestSetToRejected_Start(t *testing.T) {
	...
	state := NewSetOrderToRejected(mockRepo)

	mockPersistence.EXPECT().GetDataObject(KeyOrderId, gomock.Any()).SetArg(1, orderId)
	mockRepo.EXPECT().SetOrderStatus(orderId, string(model.REJECTED)).Return(nil)
	...
	mockPersistence.EXPECT().GetDataObject(KeyOrderId, gomock.Any()).SetArg(1, orderId)
	mockRepo.EXPECT().SetOrderStatus(orderId, string(model.REJECTED)).Return(fmt.Errorf("error"))
...
func TestSetToRejected_Decide(t *testing.T) {
	ctrl := gomock.NewController(t)
	mockRepo := repository.NewMockOrderRepository(ctrl)
	mockPersistence := iwftest.NewMockPersistence(ctrl)
	mockWorkflowCtx := iwftest.NewMockWorkflowContext(ctrl)
